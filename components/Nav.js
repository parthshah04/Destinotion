...
const Nav = ({ authed }) => (
  <nav className="fixed z-50 flex items-center justify-between flex-wrap w-full py-4 px-6 bg-gradient-to-r from-destinationBlue to-destinationOrange shadow-lg">
    ...
    <div>
      {authed && (
        <button
          disabled
          className="inline-block px-4 py-2 mr-4 text-sm text-white opacity-50 cursor-not-allowed"
        >
          Logout
        </button>
      )}
      <button
        ...
        className="inline-block px-4 py-2 border rounded text-white text-md leading-none border-white hover:border-transparent hover:text-destinationOrange hover:bg-white transition-colors duration-200 focus:outline-none"
      >
        Add Place
      </button>
    </div>
